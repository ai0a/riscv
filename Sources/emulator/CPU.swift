...
struct CPU {
    ...
    mutating func execute(_ instruction: RiscVInstruction) throws {
        ...
        case .fsw(let sourceRegister1, let sourceRegister2, let immediate):
            let address = UInt64(bitPattern: registers[Int(sourceRegister1)]) &+ UInt64(bitPattern: Int64(immediate))
            try memory.store32Bits(fpRegisters[Int(sourceRegister2)].uncheckingNanBoxedFloat.bitPattern, at: address)
        ...
        case .fmsubs(let destinationRegister, let sourceRegister1, let sourceRegister2, let sourceRegister3, _):
            ...
            fpRegisters[Int(destinationRegister)] = Double(nanBoxing: value)
            try csrs.addFloatingPointExceptions(exceptions)
        case .fnmsubs(let destinationRegister, let sourceRegister1, let sourceRegister2, let sourceRegister3, _):
            // TODO: Rounding mode
            let (value, exceptions) = fpRegisters[Int(sourceRegister1)].nanBoxedFloat.multiplyAddTrackingExceptions(-fpRegisters[Int(sourceRegister2)].nanBoxedFloat, fpRegisters[Int(sourceRegister3)].nanBoxedFloat)
            fpRegisters[Int(destinationRegister)] = Double(nanBoxing: value)
            try csrs.addFloatingPointExceptions(exceptions)
        ...
        case .fmuls(let destinationRegister, let sourceRegister1, let sourceRegister2, _):
            ...
            fpRegisters[Int(destinationRegister)] = Double(nanBoxing: value)
            try csrs.addFloatingPointExceptions(exceptions)
        case .fdivs(let destinationRegister, let sourceRegister1, let sourceRegister2, _):
            // TODO: Rounding mode
        ...
        case .fsgnjns(let destinationRegister, let sourceRegister1, let sourceRegister2):
            ...
            fpRegisters[Int(destinationRegister)] = Double(nanBoxing: value)
        case .fsgnjxs(let destinationRegister, let sourceRegister1, let sourceRegister2):
            var value = fpRegisters[Int(sourceRegister1)].nanBoxedFloat
            if value.sign == fpRegisters[Int(sourceRegister2)].nanBoxedFloat.sign {
                value = abs(value)
            } else {
        ...
        case let .fcvtsw(destinationRegister, sourceRegister, _):
            //TODO: Rounding mode
            let unconverted = UInt32(UInt64(bitPattern: registers[Int(sourceRegister)]) & 0xffffffff).signExtension()
